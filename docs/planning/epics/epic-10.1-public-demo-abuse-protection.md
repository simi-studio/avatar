...
- [x] Render the Turnstile widget only when `NEXT_PUBLIC_TURNSTILE_SITE_KEY` is set; otherwise the
      form behaves exactly as today (no widget, no token).
- [x] Pass the obtained token to `/api/generate` as a non-secret field; never log it.
- [x] i18n copy for the challenge area in `i18n/en.json` / `i18n/zh-CN.json`.

### Server verification
- [x] In `/api/generate`, when `TURNSTILE_SECRET_KEY` is set, verify the token via Cloudflare
...
- [x] The siteverify host is a fixed constant — never user-controlled — consistent with the
      "providers from a fixed allowlist" rule in `AGENTS.md`.
- [x] When the secret is unset, skip verification entirely (self-host default path, no regression).
- [x] Add an error code (e.g. `CHALLENGE_FAILED` → HTTP 403) and a normalized message; reuse the
      existing `errorResponse` flow. Missing/invalid token when enabled → reject before provider call.

### Documentation
- [x] Document the env vars and the demo-vs-self-host behavior in [cloudflare-deploy.md](../../cloudflare-deploy.md)
...
### Tests
- [x] Mocked-fetch tests: siteverify success → request proceeds; failure / missing token → 403.
- [x] Disabled path (no secret) → verification skipped, existing behavior unchanged.
- [x] `guard:secrets` proves the token and secret never reach logs.

## Acceptance

- [x] With `TURNSTILE_SECRET_KEY` unset, the demo and self-host flow are byte-for-byte unchanged
      (regression-safe default-off).
- [x] With the secret set, a missing or invalid token returns 403 **before** any provider call.
- [x] siteverify is called server-side against the fixed Cloudflare host only.
- [x] No key, token, or secret appears in logs or error bodies; full gate green
